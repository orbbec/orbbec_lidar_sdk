...
TEST(Frame, test_point_cloud_frame) {
    auto frame_impl =
        std::make_unique<ob_lidar_driver::detail::PointCloudFrameImpl>();
    EXPECT_NE(frame_impl, nullptr);
    frame_impl->setDistanceScale(1.0);
    frame_impl->setAngleScale(0.02);
    frame_impl->setFrameId(42);
    frame_impl->setTimestamp(std::chrono::nanoseconds(1000000));
    frame_impl->setFrameType(ob_lidar_driver::LidarFrameType::POINT_CLOUD);
    std::vector<uint8_t> fake_points;
    ob_lidar_driver::LidarPoint fake_point{};
    fake_point.x = 1;
    fake_point.y = 2;
    fake_point.z = 3;
    ...
    auto frame = std::make_unique<ob_lidar_driver::PointCloudFrame>(
        std::move(frame_impl));
    EXPECT_NE(frame, nullptr);
    EXPECT_FLOAT_EQ(frame->getAngleScale(), 0.02);
    EXPECT_FLOAT_EQ(frame->getDistanceScale(), 1.0);
    EXPECT_EQ(frame->frameId(), 42);
    EXPECT_EQ(frame->size(), 5 * sizeof(ob_lidar_driver::LidarPoint));
    EXPECT_EQ(frame->type(), ob_lidar_driver::LidarFrameType::POINT_CLOUD);
    EXPECT_EQ(frame->timestamp(), std::chrono::nanoseconds(1000000));
    // cast data to LidarPoint
    auto points =
        reinterpret_cast<const ob_lidar_driver::LidarPoint*>(frame->data());
    // get the first point
